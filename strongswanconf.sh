...
if [[ $place=1 ]]; then
  wget -p https://raw.githubusercontent.com/Arioxss/bash-test/footest/rtrrstrongswan
  mv rtrrstrongswan ipsec.conf
  mv ipsec.conf /etc/ipsec.conf
else
  wget -p https://raw.githubusercontent.com/Arioxss/bash-test/footest/rtrlstrongswan
